...
fun fight(a: List<Group>, b: List<Group>): Boolean {
    target.clear()
    selected.clear()
...
fun main() {
    ...
    for (boost in 1..Int.MAX_VALUE) {
        val aa = a.mapTo(mutableListOf()) { it.copy(attack = it.attack + boost) }
        val bb = b.mapTo(mutableListOf()) { it.copy() }

        var it = 0
        while (aa.isNotEmpty() && bb.isNotEmpty()) {
            if (!fight(aa, bb)) break
            cleanup(aa)
            cleanup(bb)
        }

        if (bb.isEmpty()) {
            println(aa.sumBy { it.units })
            break
        }
    }
}
